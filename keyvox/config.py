...
def save_config(path: Path, config_dict: Dict[str, Any]) -> None:
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        # Top-level scalars (e.g. version = 1) must appear before any sections
        scalars = [(k, v) for k, v in config_dict.items() if not isinstance(v, dict)]
        for key, value in scalars:
            f.write(_write_value(key, value))
        if scalars:
            f.write("\n")
